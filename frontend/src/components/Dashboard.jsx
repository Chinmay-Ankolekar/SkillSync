...
function Dashboard({ token }) {
  ...
  return (
    ...
    <div className="my-10 mx-auto flex w-screen max-w-screen-lg flex-col rounded-3xl bg-green-50 px-4">
      ...
      <h1 className="mx-auto mt-2 max-w-3xl text-center text-md font-semibold leading-tight sm:text-3xl md:text-3xl"> Seamless Job Creation, Skill-Based Shortlisting, and Top Talent Selection: Revolutionizing Recruitment.</h1>
      ...
      <div className="mx-auto mt-8 mb-20 flex w-full flex-col space-y-2 sm:w-auto sm:flex-row sm:space-y-0 sm:space-x-6">
        <button onClick={()=> navigate('/allJobsAppliedCandidates')} className="rounded-full bg-purple-900 px-10 py-3 font-medium text-white hover:opacity-80 sm:w-auto">Applied Candidates</button>
        <button onClick={() => navigate("/addjob")}  className="rounded-full border-2 border-purple-900 px-10 py-3 font-medium text-black transition hover:bg-purple-900 hover:text-white sm:w-auto">Add Jobs</button>
      </div>
    </div>
  ...
  <div class="bg-white shadow-xl shadow-gray-100 w-full max-w-4xl flex flex-col sm:flex-row gap-3 sm:items-center justify-between px-5 py-4 rounded-md">
    ...
    <div>
      <button   onClick={() => navigate(`/appliedcandidates/${job.id}`)}  class="bg-purple-900 text-white font-medium px-4 py-2 rounded-md flex gap-1 items-center">
        Applied Candidates
        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
